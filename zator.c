...
void save_img_var(Var *v, const char *relpath) {
    if (!v || v->type != VAR_IMAGE || !v->img.b64) return;
    ...
    FILE *f = fopen("tmp_image.b64", "wb");
    if (!f) { fprintf(stderr, "save_img: cannot open tmp_image.b64\n"); return; }
    fwrite(v->img.b64, 1, strlen(v->img.b64), f);
    fclose(f);

    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "certutil -decode tmp_image.b64 \"%s\" >nul 2>&1", abs);
    int rc = system(cmd);
    (void)rc;
...
/* Extract first base64 string from "images":["BASE64",...] */
char *extract_first_image_b64(const char *json) {
    ...
    if (!start_bracket) return NULL;
    
    char *first_quote = strchr(start_bracket, '"');
    if (!first_quote) return NULL;
    
    first_quote++; // Move past the quote
    
    const char *end_quote = first_quote;
    while (*end_quote) {
        if (*end_quote == '"' && *(end_quote - 1) != '\\') break;
        end_quote++;
    }
    
    size_t length = end_quote - first_quote;
    char *b64 = (char*)malloc(length + 1);
    if (!b64) return NULL;
    
    strncpy(b64, first_quote, length);
    b64[length] = 0;
    
    // Remove any escape characters from the base64 string
    ...
    while (i < line_count) {
        ...
        if (starts_with(l, "context")) {
            ...
            if (sscanf(l, "context = \"%[^\"]\"", val) == 1) {
                strncpy(context_str, val, sizeof(context_str)-1);
        ...
        if (starts_with(l, "server")) {
            ...
            if (sscanf(l, "server = \"%[^\"]\"", val) == 1) {
                strncpy(api_server, val, sizeof(api_server)-1);
        ...
        if (starts_with(l, "var ")) {
            ...
            if (strstr(l, "generate_text(")) {
                ...
                if (sscanf(l, "var %*s = generate_text(%127[^,], context, %d)", pmt, &tokens) >= 1) {
                    ...
                    if (*pp == '"') {
                        // literal
                        char tmp[32768]; if (sscanf(pp, "\"%[^\"]\"", tmp) == 1) strncpy(prompt_text, tmp, sizeof(prompt_text)-1);
                    } else {
                        Var *vp = get_var(pp);
            ...
            if (sscanf(l, "var %*s = \"%[^\"]\"", lit) == 1) {
                v->type = VAR_STRING;
                strncpy(v->sv, lit, sizeof(v->sv)-1);
        ...
        if (starts_with(l, "array_push_demo(")) {
            char val[512];
            if (sscanf(l, "array_push_demo(\"%511[^\"]\")", val) == 1) array_push_demo(val);
            i++; continue;
        }
